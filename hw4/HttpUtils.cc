...
bool IsPathSafe(const string &rootdir, const string &testfile) {
  ...
  // STEP 1
  char rootbuffer[PATH_MAX], testbuffer[PATH_MAX];

  if (!realpath(rootdir.c_str(), rootbuffer) ||
      !realpath(testfile.c_str(), testbuffer)) {
        return false;
  }

  if (strlen(testbuffer) <= strlen(rootbuffer)) {
    return false;
  }
  // ensure test if part of root
  if (strncmp(rootbuffer, testbuffer, strlen(rootbuffer)) != 0) {
    return false;
  }
